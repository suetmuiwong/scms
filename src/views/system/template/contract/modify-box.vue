<template>
  <div>
    <el-dialog
      :title="boxParams.type === 'edit' ? '编辑合同模板': '新建合同模板'"
    ...
    >
      <div slot="title" class="dialog-header">
        <div class="dialog-custom-header">
          <i class="line-icon"></i>
          <span class="header-title">{{boxParams.type === 'edit' ? '编辑合同模板': '新建合同模板'}}</span>
        </div>
        <div>
      ...
      <el-collapse accordion  v-model="activeNames">
        <el-collapse-item name="1">
          ...
          <div class="collapse-content">
            <el-form :model="form" ref="form" label-width="150px" class="basic-form" size="mini" :rules="rules">
              <div class="form-container">
                <div class="form-wrapper">
                  <el-form-item label="合同模板主题：" prop="contractTemplateSubject">
                    <el-input v-model="form.contractTemplateSubject" autocomplete="off" clearable required></el-input>
                  </el-form-item>
...
export default {
  ...
  watch: {
    ...
    boxParams: {
      ...
      handler (val) {
        if (val) {
          if (val.type === 'edit') {
            this.fetchDetailData(val.data.contractTemplateId)
          } else {
            this.form = {
              contractTemplateSubject: '',
              status: '',
  ...
  methods: {
    fetchDetailData (id) {
      FetchContractTemplate('get', id, true).then((res) => {
        this.form = {
          contractTemplateSubject: res.contractTemplateSubject,
          status: res.status,
          creator: res.creator,
    ...
    submitForm (formName) {
      if (!this.form.contractTemplateSubject) {
        this.$message({
          message: '请填写合同模板主题!',
          type: 'warning',
          duration: 3 * 1000
      ...
      let params = {
        contractTemplateSubject: this.form.contractTemplateSubject,
      ...
      if (this.boxParams.type === 'add') {
        FetchContractTemplate('post', params).then((res) => {
          this.$message({
            message: '新建合同模板成功！',
            type: 'success',
            duration: 3 * 1000
        ...
        params.id = this.boxParams.data.contractTemplateId
        FetchContractTemplate('put', params).then((res) => {
          this.$message({
            message: '修改合同模板成功！',
            type: 'success',
            duration: 3 * 1000
